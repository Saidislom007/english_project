...
export default function WritingTest() {
  ...
  const updateWritingSkill = async (ieltsScore) => {
    if (isNaN(ieltsScore)) return; // ✅ NaN bo‘lsa, hech narsa qilmaydi

    try {
      const response = await fetch(
        `http://192.168.1.45:5050/api/auth/update-skill/${userId}`,
        {
          method: "PUT",
  ...
  const checkWriting = async () => {
    try {
      const response = await fetch("http://192.168.1.45:5050/api/check-writing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.feedback) {
        ...
        const parsedScore = parseFloat(data.score); // ✅ Son formatiga o'tkazish
        setScore(!isNaN(parsedScore) ? parsedScore : "Not available");

        if (!isNaN(parsedScore)) {
          updateWritingSkill(parsedScore); // ✅ Faqat raqam yuboriladi
        }
      } else {
      ...
      setFeedback("Xatolik yuz berdi, qaytadan urinib ko‘ring.");
      setScore(null);
    }
  };
